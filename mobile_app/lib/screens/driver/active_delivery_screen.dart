...
class _ActiveDeliveryScreenState extends State<ActiveDeliveryScreen> {
      ...
      try {
        ...
        if (!_locationService.isTracking || _locationService.activeOrderId != widget.orderId) {
          if (order.status == OrderStatus.onTheWay) {
            ...
            await _locationService.startTracking(widget.orderId, isKermes: false);
          }
        }

        // 2. Start compass if order has precise pin
        if (!order.hasPrecisePin || order.deliveryPinLat == null || order.deliveryPinLng == null) return;
  ...
  Future<void> _completeDelivery(LokmaOrder orderSnapshot) async {
    ...
    if (isCardOnDelivery) {
      final currentStatus = (orderSnapshot.paymentStatus == 'paid' || orderSnapshot.paymentStatus == 'collected') ? 'collected' : 'pending';
    ...
    return Scaffold(
      ...
      body: StreamBuilder<LokmaOrder?>(
        ...
        builder: (context, snapshot) {
          ...
          // Auto-pop if delivery was completed (prevents stale UI after stream update)
          // _hasPopped guard: if _completeDelivery already popped, skip to avoid double-pop crash
                      ...
                      SizedBox(
                        ...
                        child: isOnTheWay 
                          ? ElevatedButton.icon(
                              ...
                              label: Text(
                                _isCompleting ? 'TAMAMLANIYOR...' : 'TESLIMAT TAMAMLANDI',
                                style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 16),
                              ),
                              style: ElevatedButton.styleFrom(
                                backgroundColor: _isCompleting ? Colors.grey : Colors.green,
                                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(26)),
                              ),
                            )
                          : isReady

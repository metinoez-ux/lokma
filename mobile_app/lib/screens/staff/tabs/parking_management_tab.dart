...
class _ParkingManagementTabState extends State<ParkingManagementTab> {
  ...
  // Durumu dongu gibi degistir: belirsiz -> bos -> dolu -> belirsiz
  Future<void> _toggleStatus(int index) async {
    HapticFeedback.mediumImpact();
    final current = _parkingList[index]['status'] as String?;
    final String? next;
    ...
    try {
      ...
      final list = List<Map<String, dynamic>>.from(
        (data['parkingLocations'] as List<dynamic>? ?? [])
            .map((e) => Map<String, dynamic>.from(e as Map)),
      );
      ...
      if (index < list.length) {
        if (next != null) {
          ...
          list[index].remove('statusUpdatedAt');
          list[index].remove('statusUpdatedBy');
        }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Guncelleme hatasi: $e'),
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      // GPS ve Kermes adresi butonlari
                      const Text(
                        'Hizli Ekle',
                        style: TextStyle(
                                  ...
                                  : () async {
                                      ...
                                      try {
                                        ...
                                        if (marks.isNotEmpty) {
                                          ...
                                          setSheet(() {
                                            streetCtrl.text =
                                                '${m.street ?? ''} ${m.subThoroughfare ?? ''}'
                                                    .trim();
                                            cityCtrl.text = m.locality ??
                                        ...
                                        ScaffoldMessenger.of(context)
                                          ...
                                          const SnackBar(
                                            content: Text(
                                                'GPS konumu alindi, kontrol edin'),
                      ...
                      _Field(
                        controller: noteCtrl,
                        label: 'Not',
                        hint: 'Orn: Cadde boyu park edilebilir',
                        maxLines: 3,
                        isDark: isDark,
                      ),
                      // Secili resimler
                          ...
                          : () async {
                              if (streetCtrl.text.isEmpty ||
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text(
                                        'Lutfen sokak ve sehir bilgisi girin'),
                              ...
                              try {
                                ...
                                for (int i = 0; i < images.length; i++) {
                                  final ref = FirebaseStorage.instance
                                      .ref()
                                      .child(
                                          'kermes/${widget.kermesId}/parking/${DateTime.now().millisecondsSinceEpoch}_$i.jpg');
                                ...
                                if (mounted) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text('Park alani eklendi'),
                                      backgroundColor: Colors.green,
                                    ),
                                  );
                                }
                          ...
                          children: [
                            ...
                            _StatusChip(
                              ...
                              onTap: () async {
                                ...
                                if (!doc.exists) return;
                                final list = List<Map<String, dynamic>>.from(
                                  (doc.data()!['parkingLocations'] as List? ?? [])
                                      .map((e) => Map<String, dynamic>.from(e as Map)),
                                );
                                if (i < list.length) {
                                  list[i].remove('status');
                                  list[i].remove('statusUpdatedAt');
                                  await docRef.update({'parkingLocations': list});
                                }
                              },
                            ),

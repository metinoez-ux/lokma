...
class _RestoranScreenState extends ConsumerState<RestoranScreen> {
    ...
    return WalletBusinessCard(
      ...
      onTap: () {
        ...
        if (!isAvailable) {
          ...
          final hasReservation = data['hasReservation'] == true;
          if (_deliveryMode == 'masa' && hasReservation) {
            _showMasaActionSheet(id, name, data);
          } else {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 24),
                  Text(
                    tr('marketplace.masa_action_title'),
                    style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600),
                    textAlign: TextAlign.center,
                  ),
                  const SizedBox(height: 24),
                  // Masa Rezervasyonu Tile
                  InkWell(
                    onTap: () {
                      Navigator.pop(ctx);
                      Navigator.push(
                        ...
                        children: [
                          Container(
                            ...
                            child: SvgPicture.asset(
                              ...
                              width: 24,
                              height: 24,
                              colorFilter: const ColorFilter.mode(Color(0xFFE2211C), BlendMode.srcIn),
                            ),
                          ),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  tr('marketplace.masa_action_reserve'),
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  tr('marketplace.masa_action_browse'),
                                  style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600),
                                ),
                                const SizedBox(height: 4),
                                Text(
  ...
  Future<void> _handleMasaQrScanned(String tableNum, String? businessId) async {
    ...
    // --- Continuous Tab (Phase 2): Check for active reservation tab ---
    try {
      ...
      if (user != null) {
        ...
        final activeTab = resQuery.docs.where((doc) {
          final tabStatus = doc.data()['tabStatus'] as String?;
          final status = doc.data()['status'] as String?;
          return (tabStatus == 'pre_ordered' || tabStatus == 'seated') && status == 'confirmed';
        }).firstOrNull;

        if (activeTab != null) {
          if (!mounted) return;
          _showTabCheckInDialog(tableNum, businessId!, businessName ?? tr('marketplace.unnamed_store'), activeTab.id);
          return;
        }
      }
    } catch (e) {
      debugPrint('Error checking active reservation tab: $e');
    }
  ...
  /// 🕒 Check into continuous tab/reservation (Phase 2)
  void _showTabCheckInDialog(String tableNum, String businessId, String businessName, String tabId) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            const Text(
              'Aktif Rezervasyonunuz Bulundu',
              style: TextStyle(fontSize: 20, fontWeight: FontWeight.w600),
              textAlign: TextAlign.center,
            ),
            const SizedBox(height: 8),
            Text(
              'Bu restoranda bugün için bir rezervasyonunuz/ön siparişiniz var. Masaya giriş yapıp adisyonunuz üzerinden siparişe devam etmek ister misiniz?',
              textAlign: TextAlign.center,
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () {
                  Navigator.pop(ctx);
                  _setScannedTable(tableNum, businessId, businessName);
                  // Update tab Status to 'seated' and bind the physical table
                  FirebaseFirestore.instance
                      .collection('businesses')
                      .doc(businessId)
                      .collection('reservations')
                  ...
                  context.push('/kasap/$businessId?mode=masa&table=$tableNum&reservationTabId=$tabId');
                },
                style: ElevatedButton.styleFrom(
                  backgroundColor: lokmaPink,
                  shape: RoundedRectangleBorder(
                ...
                child: const Text('Masaya Giriş Yap (Adisyona Devam)', 

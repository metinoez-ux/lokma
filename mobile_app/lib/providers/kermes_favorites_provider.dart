...
/// Kermes Favorites Provider - Hybrid Storage
...
final kermesFavoritesProvider = NotifierProvider<KermesFavoritesNotifier, List<String>>(KermesFavoritesNotifier.new);

class KermesFavoritesNotifier extends Notifier<List<String>> {
  static const String _localPrefsKey = 'favorite_kermes';
  ...
  Future<void> _loadFromFirestore() async {
    try {
      ...
      if (doc.exists) {
        final data = doc.data();
        final favorites = (data?['favoriteKermes'] as List<dynamic>?)
            ?.map((e) => e.toString())
            .toList() ?? [];
        state = favorites;
  ...
  Future<void> toggleFavorite(String kermesId) async {
    List<String> newFavorites;
    
    if (state.contains(kermesId)) {
      newFavorites = state.where((id) => id != kermesId).toList();
    } else {
      newFavorites = [...state, kermesId];
    }
  ...
  /// Check if a kermes is favorited
  bool isFavorite(String kermesId) {
    return state.contains(kermesId);
  }

...
const log = fs.readFileSync('analyze_theme5.log', 'utf8');
...
for (const file of Object.keys(constErrors)) {
    ...
    for (const errorLine of errLines) {
        ...
        for (let i = start; i >= Math.max(0, start - 30); i--) {
            ...
            if (constMatch) {
                // Remove the LAST occurrence of "const " on the line, because it's usually `const Padding(child: const Text())`
                const idx = lineStr.lastIndexOf('const ');
                if (idx !== -1) {
                    linesArr[i] = lineStr.substring(0, idx) + lineStr.substring(idx + 6);
                    found = true;
                    console.log(`Final Sweep: Removed const in ${file}:${i + 1} for error at line ${errorLine}`);
                    break;
                }
            }
...
console.log('Final Const Sweep applied.');

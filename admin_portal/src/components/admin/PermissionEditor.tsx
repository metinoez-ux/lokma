...
function PermissionToggle({
 ...
 ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-muted/30 dark:hover:bg-card'}
 ...
 <div className="text-sm font-medium text-foreground dark:text-gray-100 flex items-center gap-1.5">
 {label}
 {isOverride && (
 ...
 {description && (
 <div className="text-xs text-muted-foreground/80 dark:text-muted-foreground truncate">{description}</div>
 )}
 </div>
 </label>
 );
...
function ModuleSection({
 ...
 return (
 <div className="border border-border/50 dark:border-border rounded-xl overflow-hidden">
 {/* Module Header */}
 <div
 ...
 <span className="text-xs text-muted-foreground/80 dark:text-muted-foreground ml-1">
 ({actions.filter(([actionId]) => permissions[`${moduleId}.${actionId}` as PermissionKey] === true).length}/{actions.length})
 </span>
 </div>
 {!readOnly && (
 <div className={`w-3 h-3 rounded-full border-2 transition-colors
 ${allChecked ? 'bg-green-500 border-green-500' : someChecked ? 'bg-amber-400 border-amber-400' : 'border-border dark:border-gray-600'}
 `} />
 )}
 </div>

 {/* Action Toggles */}
 <div className="px-3 py-2 space-y-0.5 bg-background dark:bg-background">
...
export default function PermissionEditor({
 ...
 <p className="text-sm text-muted-foreground/80 dark:text-muted-foreground mt-0.5">
 ...
 {/* Group Selector */}
 {!readOnly && (
 <div className="bg-muted/30 dark:bg-card/50 rounded-xl p-4">
 <label className="block text-xs font-semibold text-muted-foreground dark:text-foreground uppercase tracking-wide mb-2">
 ...
 : 'bg-background dark:bg-background border-border/50 dark:border-border hover:border-gray-400 dark:hover:border-gray-500 text-foreground/80 dark:text-foreground'
 }
 `}
 >
 ...
 <p className="mt-2 text-xs text-muted-foreground/80 dark:text-muted-foreground italic">
 ...
 return (
 <div key={category} className="border border-border/50 dark:border-border rounded-xl overflow-hidden">
 {/* Category Header */}
 <button
 onClick={() => toggleCategory(category)}
 className="w-full flex items-center justify-between px-4 py-3 bg-muted dark:bg-card hover:bg-muted/50 dark:hover:bg-gray-700 transition-colors"
 >
 <span className="flex items-center gap-2 font-bold text-sm text-foreground/90 dark:text-gray-200">
 <span>{icon}</span>
 <span>{t(labelKey)}</span>
 </span>
 <svg
 className={`w-4 h-4 text-muted-foreground/80 transition-transform ${isExpanded ? 'rotate-180' : ''}`}
 fill="none"
 viewBox="0 0 24 24"
 stroke="currentColor"
 >
 ...
 {/* Modules */}
 {isExpanded && (
 <div className="p-3 space-y-3 bg-background dark:bg-background">
 {modules.map((mod) => (
 <ModuleSection
 key={mod.id}
 moduleId={mod.id}

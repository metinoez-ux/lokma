...
const TimeInput: React.FC<{
 ...
 <span className="text-muted-foreground/80 text-[10px] uppercase mb-0.5">{label}</span>
 <input
 ref={inputRef}
 type="text"
 value={value}
 ...
 ${disabled
 ? 'bg-gray-700/50 text-muted-foreground/80 border-gray-700 cursor-not-allowed'
...
const [schedule, setSchedule] = useState<DaySchedule[]>(value?.length > 0 ? value : defaultSchedule);
 ...
 label={t('saat')}
 />
 <span className="text-muted-foreground/80 text-xl font-light">:</span>
 <TimeInput

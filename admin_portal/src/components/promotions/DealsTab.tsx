...
export default function DealsTab({ businessId, isSuperAdmin, businesses, deals, setDeals, loaded, setLoaded, setAllBusinesses }: Props) {
 ...
 <p className="text-muted-foreground/80 text-sm mt-3">Nutzen Sie den <span className="text-amber-400 font-medium">+ Angebot erstellen</span> Button oben rechts</p>
 </div>
 ) : (
 ...
 {deal.validUntil && <p className="text-xs text-muted-foreground/80">📅 {deal.validFrom} → {deal.validUntil}</p>}
 </div>
 ...
 <div className="w-11 h-6 bg-gray-600 rounded-full peer peer-checked:bg-amber-500 after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-background after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-5" />
 </label>

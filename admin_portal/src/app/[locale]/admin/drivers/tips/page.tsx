...
export default function DriverTipsOverviewPage() {
 ...
 if (loading) {
 return (
 <div className="min-h-screen bg-card dark:bg-background flex items-center justify-center">
 <div className="text-center">
 <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-amber-500 mx-auto mb-4"></div>
 <p className="text-muted-foreground/80 dark:text-muted-foreground">{t('loading')}</p>
 </div>
 </div>
 );
 }
 ...
 return (
 <div className="min-h-screen bg-muted/30 dark:bg-background text-foreground p-6">
 ...
 <p className="text-muted-foreground/80 dark:text-muted-foreground text-sm mt-1">
 {t('subtitle')}
 </p>
 </div>
 ...
 className="bg-muted/50 dark:bg-gray-700 text-foreground/80 px-4 py-2 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 flex items-center gap-2 text-sm"
 >
 Ana Ekran (Benutzer)
 </Link>
 <Link
 ...
 : 'bg-muted/50 dark:bg-card text-muted-foreground dark:text-foreground hover:bg-gray-300 dark:hover:bg-gray-700'
 }`}
 >
 {p.label}
 </button>
 ...
 {/* Tabs */}
 <div className="flex gap-1 mb-6 bg-muted/50 dark:bg-card rounded-lg p-1 w-fit">
 <button
 onClick={() => { setActiveTab('overview'); setSelectedDriverId(null); }}
 className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
 activeTab === 'overview' ? 'bg-amber-500 text-foreground' : 'text-muted-foreground/80 dark:text-muted-foreground hover:text-foreground dark:hover:text-white'
 }`}
 >
 {t('tab_drivers')}
 </button>
 ...
 className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
 activeTab === 'details' ? 'bg-amber-500 text-foreground' : 'text-muted-foreground/80 dark:text-muted-foreground hover:text-foreground dark:hover:text-white'
 }`}
 >
 {t('tab_details')}
 </button>
 ...
 className="w-full max-w-md bg-card dark:bg-card border border-border dark:border-border rounded-lg px-4 py-2 text-foreground focus:ring-2 focus:ring-amber-500"
 />
 </div>

 {/* ---- TAB: OVERVIEW (Driver Summary Table) ---- */}
 {activeTab === 'overview' && (
 <div className="bg-card dark:bg-card rounded-xl overflow-hidden shadow-sm">
 <div className="overflow-x-auto">
 <table className="w-full text-sm">
 <thead>
 <tr className="border-b border-border/50 dark:border-border text-left text-muted-foreground/80 dark:text-muted-foreground">
 ...
 <tr>
 <td colSpan={9} className="px-4 py-8 text-center text-muted-foreground/80">
 {searchQuery ? t('no_search_results') : t('no_driver_data')}
 </td>
 </tr>
 ) : (
 filteredDrivers.map(driver => (
 <tr key={driver.driverId} className="border-b border-border/30 dark:border-border/50 hover:bg-muted/30 dark:hover:bg-gray-700/30 transition-colors">
 <td className="px-4 py-3">
 <div className="font-medium text-foreground ">{driver.driverName}</div>
 <div className="text-xs text-muted-foreground dark:text-muted-foreground/80">{driver.driverEmail}</div>
 </td>
 <td className="px-4 py-3 text-right font-bold text-amber-800 dark:text-amber-400">
 {formatCurrency(driver.totalTips)}
 </td>
 <td className="px-4 py-3 text-center text-muted-foreground dark:text-foreground">{driver.totalDeliveries}</td>
 <td className="px-4 py-3 text-center text-green-600 dark:text-green-400">{driver.tippedDeliveries}</td>
 <td className="px-4 py-3 text-right text-muted-foreground dark:text-foreground">{formatCurrency(driver.avgTip)}</td>
 ...
 driver.payoutFrequency === 'manual' ? 'bg-muted dark:bg-gray-700 text-muted-foreground dark:text-foreground' :
 'bg-muted dark:bg-gray-700 text-muted-foreground dark:text-muted-foreground/80'
 }`}>
 ...
 !selectedDriverId ? 'bg-amber-500 text-foreground' : 'bg-muted/50 dark:bg-gray-700 text-muted-foreground dark:text-foreground hover:bg-gray-300 dark:hover:bg-gray-600'
 }`}
 >
 {t('all_drivers')}
 </button>
 ...
 className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${
 selectedDriverId === d.driverId ? 'bg-amber-500 text-foreground' : 'bg-muted/50 dark:bg-gray-700 text-muted-foreground dark:text-foreground hover:bg-gray-300 dark:hover:bg-gray-600'
 }`}
 >
 ...
 <tr>
 <td colSpan={isSuperAdminUser ? 9 : 7} className="px-4 py-8 text-center text-muted-foreground/80">
 {t('no_entries')}
 </td>
 </tr>
 ) : (
 filteredEntries.slice(0, 200).map(entry => (
 <tr key={entry.orderId} className="border-b border-border/30 dark:border-border/50 hover:bg-muted/30 dark:hover:bg-gray-700/30 transition-colors">
 <td className="px-4 py-3 text-muted-foreground dark:text-foreground">{formatDate(entry.deliveredAt)}</td>
 <td className="px-4 py-3 text-muted-foreground/80 dark:text-muted-foreground">{formatTime(entry.deliveredAt)}</td>
 <td className="px-4 py-3 text-foreground dark:text-foreground font-medium">{entry.driverName}</td>
 {isSuperAdminUser && <td className="px-4 py-3 text-foreground/80 dark:text-gray-200 font-medium">{entry.customerName || '—'}</td>}
 {isSuperAdminUser && <td className="px-4 py-3 text-muted-foreground/80 dark:text-muted-foreground font-mono text-xs">{entry.orderId.substring(0, 12)}...</td>}
 <td className="px-4 py-3 text-muted-foreground dark:text-foreground">{entry.city || '—'}</td>
 <td className="px-4 py-3 text-right text-muted-foreground dark:text-foreground">{formatCurrency(entry.orderTotal)}</td>
 <td className="px-4 py-3 text-center">
 ...
 <div className="px-4 py-3 text-center text-muted-foreground/80 text-sm border-t border-border/50 dark:border-border">

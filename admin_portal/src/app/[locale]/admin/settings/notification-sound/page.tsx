...
export default function NotificationSoundPage() {
 ...
 >
 <span className="absolute top-3 right-3 text-xs text-muted-foreground font-mono">{sound.freq} Hz</span>
 <div className="text-2xl mb-2">{sound.icon}</div>
 <h3 className="font-bold text-foreground">{sound.name}</h3>
 <p className="text-xs text-muted-foreground/80 mt-1">{sound.desc}</p>

 {/* Action buttons */}
 <div className="flex items-center gap-2 mt-3">
 <button
 onClick={(e) => { e.stopPropagation(); handlePlay(sound); }}
 className="text-xs bg-muted/50 hover:bg-gray-300 text-foreground/90 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-100 px-3 py-1.5 rounded-lg transition"
 >
 ▶ {t('ns_anhoeren')}
 </button>
 {!isActive && (

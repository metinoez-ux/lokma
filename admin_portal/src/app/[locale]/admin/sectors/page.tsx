...
const [sectors, setSectors] = useState<Sector[]>([]);
 ...
 <div key={cat.value} className="flex items-center gap-2 text-sm text-muted-foreground">
 ...
 : 'bg-[#1a1a1a]/50 border-border/50 opacity-60'
 }`}
 ...
 <p className="text-xs text-muted-foreground/80">{sector.description}</p>
 </div>
 </div>
 ...
 <span className="text-xs text-muted-foreground/80">{t('sira')} {sector.sortOrder}</span>
 </div>
 </div>
 ))}
 </div>
 ...
 <div className="bg-[#1a1a1a] rounded-2xl p-6 w-full max-w-lg border border-border max-h-[90vh] overflow-y-auto">
 <div className="flex items-center justify-between mb-6">
 <h2 className="text-xl font-bold">
 {isNewSector ? t('yeni_sektor_ekle') : `${editingSector.icon} ${editingSector.label}`}
 </h2>
 <button onClick={() => { setEditingSector(null); setIsNewSector(false); }} className="text-muted-foreground/80 hover:text-white">
 ✕
 </button>
 </div>
 ...
 className="w-full px-3 py-2 bg-[#0f0f0f] border border-border rounded-lg focus:border-red-500 outline-none"
 />
 </div>
 )}
 ...
 <span className="text-sm font-medium text-muted-foreground">{t('aktif')}</span>
 <div
 ...
 >
 <div className={`w-5 h-5 rounded-full bg-card transition-transform m-0.5 ${editingSector.isActive ? 'translate-x-6' : 'translate-x-0'
 }`} />
 </div>
 </label>
 </div>
 ...
 className="flex-1 px-4 py-2 border border-border rounded-lg hover:bg-card"
 >
 İptal
 </button>
 <button

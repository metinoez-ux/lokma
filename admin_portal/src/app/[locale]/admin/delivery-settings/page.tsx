...
const { admin, loading: adminLoading } = useAdmin();
 ...
 className="px-4 py-2 bg-muted/50 hover:bg-gray-300 text-foreground/90 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-100 rounded-lg transition"
 >
 İptal
 </button>
 <button
 ...
 <span className="text-muted-foreground/80 italic text-sm">{t('calisma_saatleri_henuz_belirlenmemis')}</span>
 {!isEditing && (
 <button
 onClick={() => setIsEditing(true)}
 ...
 <span className="text-xs text-muted-foreground/80">(Opsiyonel)</span>
 </h5>
 ...
 />
 <p className="text-xs text-muted-foreground/80 mt-1">{t('bos_acilis_saati')}</p>
 </div>
 ...
 className="w-full bg-gray-700 text-white px-3 py-2 rounded-lg mt-1 disabled:opacity-50 [color-scheme:dark]"
 />
 <p className="text-xs text-muted-foreground/80 mt-1">{t('bos_kapanis_saati')}</p>
 </div>
 ...
 <p className="text-xs text-muted-foreground/80 mt-1">{t('0_her_zaman_teslimat_ucreti_uygulanir')}</p>
 </div>
 ...
 <h5 className="text-foreground font-medium mb-3 flex items-center gap-2">
 {t('komisyon_oranlari')}
 <span className="text-xs text-muted-foreground/80">({planName} {t('planiniza_gore')}</span>
 </h5>

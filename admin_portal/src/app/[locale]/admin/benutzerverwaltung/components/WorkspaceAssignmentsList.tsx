...
export function WorkspaceAssignmentsList({ assignments, onChange, businesses, kermesEvents, isSuperAdmin }: WorkspaceAssignmentsListProps) {
  ...
  const handleAdd = () => {
    ...
    if (assignments.find(a => a.id === selectedEntityId && a.role === selectedRole)) {
      alert(t('rolZatenEklendi', { defaultValue: "Bu rol ve işletme/kermes daha önce eklenmiş." }));
      return;
    }
    ...
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        <label className="block text-sm font-medium text-foreground">
          {t('isletme_atamalari') || 'İşletme / Kermes Atamaları'} ({assignments.length})
        </label>
        {isSuperAdmin && (
          <button 
            type="button" 
          ...
          >
            {t('rolEkle', { defaultValue: '+ Rol Ekle' })}
          </button>
        )}
      </div>
      ...
      {showAdd && isSuperAdmin && (
        <div className="mt-3 bg-muted/40 border border-border p-4 rounded-xl space-y-4 shadow-inner">
          <div className="flex justify-between items-center border-b border-border pb-2">
            <h4 className="font-semibold text-sm">{t('yeniRolAtamasi', { defaultValue: 'Yeni Rol Ataması' })}</h4>
            <button type="button" onClick={() => setShowAdd(false)} className="text-muted-foreground hover:text-foreground">{t('kapat', { defaultValue: 'Kapat' })}✕</button>
          </div>
          
          <div className="space-y-3">
            <div>
               <label className="block text-xs font-medium text-muted-foreground mb-1">{t('isletme_kermes', { defaultValue: 'İşletme veya Kermes' })}</label>
                  ...
                  {!isDropdownOpen ? (
                      ...
                      >
                          <span className={selectedEntityId ? "text-foreground" : "text-muted-foreground truncate"}>
                              {selectedEntityId 
                                  ? allEntities.find(b => b.id === selectedEntityId)?.name || t('isletme_kermes_secin', { defaultValue: 'İşletme / Kermes Seçin...' })
                                  : t('isletme_kermes_secin', { defaultValue: 'İşletme / Kermes Seçin...' })
                              }
                          </span>
                      ...
                      <div className="absolute top-full left-0 mt-1 w-full bg-background border border-border rounded-lg shadow-xl z-50">
                          <div className="p-2 border-b border-border flex items-center gap-2">
                              ...
                              <input 
                                  type="text" 
                                  autoFocus
                                  placeholder={t('ara', { defaultValue: 'kermes/işletme ara...' })} 
                          ...
                          <div className="max-h-60 overflow-y-auto">
                              {allEntities
                                  ...
                                  .map(b => (
                                      <div 
                                          key={b.id} 
                                          onClick={() => { setSelectedEntityId(b.id); setIsDropdownOpen(false); setSearchQuery(''); }}
                              ...
                              {allEntities.filter(b => {
                                  ...
                                  <div className="p-4 text-center text-xs text-muted-foreground">{t('sonucBulunamadi', { defaultValue: 'Sonuç bulunamadı.' })}</div>
                              )}
                          </div>
                      </div>
                  )}
            ...
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">{t('roldekiKonum', { defaultValue: 'Bu iş yerindeki Rolü' })}</label>
              <select 
                value={selectedRole}
            ...
            >
              {t('listeyeEkle', { defaultValue: 'Listeye Ekle' })}
            </button>
          </div>
        </div>
      )}

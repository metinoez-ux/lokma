...
export default function BenutzerverwaltungPage() {
    ...
    const [authProviders, setAuthProviders] = useState<Record<string, string[]>>({});
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // 4. Fetch Auth Providers
            if (isSuperAdmin) {
                try {
                    const token = await auth.currentUser?.getIdToken();
                    const provResp = await fetch('/api/admin/auth-providers', {
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                    if (provResp.ok) {
                        const provData = await provResp.json();
                        if (provData.providerMap) {
                            setAuthProviders(provData.providerMap);
                        }
                    }
                } catch (err) {
                                        ...
                                        return (
                                            <tr key={user.id} className="hover:bg-muted/30 transition-colors group">
                                                ...
                                                <td className="px-6 py-4">
                                                    ...
                                                    {isSuperAdmin && authProviders[user.id] && authProviders[user.id].length > 0 && (
                                                        <div className="flex flex-wrap items-center gap-1 mt-1.5">
                                                            {authProviders[user.id].includes('google.com') && (
                                                                <span title="Google ile Kayıt" className="text-[10px] bg-white border border-gray-200 dark:border-gray-700 dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-medium px-1.5 py-0.5 rounded flex items-center gap-1">
                                                            ...
                                                            {authProviders[user.id].includes('apple.com') && (
                                                                <span title="Apple ile Kayıt" className="text-[10px] bg-black text-white px-1.5 py-0.5 font-medium rounded flex items-center gap-1">
                                                                     Apple
                                                                </span>
                                                            )}
                                                            {authProviders[user.id].includes('phone') && (
                                                                <span title="SMS ile Kayıt" className="text-[10px] bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 font-medium px-1.5 py-0.5 rounded flex items-center gap-1">
                                                                    💬 SMS
                                                                </span>
                                                            )}
                                                            {authProviders[user.id].includes('password') && (
                                                                <span title="Email/Şifre ile Kayıt" className="text-[10px] bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400 font-medium px-1.5 py-0.5 rounded flex items-center gap-1">
                                                                    ✉️ E-mail
                                                                </span>
                                                            )}
                                                        </div>

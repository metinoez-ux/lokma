...
export default function ReservationsPage() {
 ...
 className={`h-10 px-4 rounded-lg font-medium text-sm transition-all flex items-center gap-2 ${showTableManagement ? 'bg-amber-600 text-white shadow-md' : 'bg-muted/50 dark:bg-gray-800 text-foreground/90 dark:text-gray-200 border border-border dark:border-gray-700 hover:bg-gray-300 dark:hover:bg-gray-700'}`}
 >
 ...
 <span className="text-xs text-muted-foreground/80 font-medium">{group.reservations.length} rezervasyon</span>
 </div>
 ...
 {selectedReservation.status === 'confirmed' && (
 <div className="flex flex-col gap-3">
 <button onClick={() => confirmWithCards(selectedReservation, Array.from(selectedCards).sort((a,b)=>a-b))} disabled={selectedCards.size === 0} className={`w-full py-4 text-center rounded-xl font-bold transition-all active:scale-[0.98] ${selectedCards.size > 0 ? 'bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-900/20' : 'bg-gray-800 text-muted-foreground/80 cursor-not-allowed opacity-50'}`}>
 ...
 <button disabled={!cancelReason} onClick={handleCancel} className={`flex-[2] py-3.5 rounded-xl font-bold transition ${!cancelReason ? 'bg-gray-700 text-muted-foreground/80 cursor-not-allowed' : 'bg-red-600 hover:bg-red-500 text-white shadow-lg shadow-red-900/30'}`}>İptali Onayla</button>
 </div>
 </div>
 </div>
 )}
 ...
 <button disabled={!rejectReason} onClick={handleReject} className={`flex-[2] py-3.5 rounded-xl font-bold transition ${!rejectReason ? 'bg-gray-700 text-muted-foreground/80 cursor-not-allowed' : 'bg-red-600 hover:bg-red-500 text-white shadow-lg shadow-red-900/30'}`}>Reddetmeyi Onayla</button>
 </div>
 </div>
 </div>
 )}

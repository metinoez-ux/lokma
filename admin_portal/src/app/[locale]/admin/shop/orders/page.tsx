...
export default function ShopOrdersPage() {
 ...
 <span className="text-muted-foreground/80 text-sm ml-2">{formatDate(order.createdAt)}</span>
 </div>
 ...
 <div className="text-muted-foreground/80">+{order.items.length - 3} {t('daha_fazla')}</div>
 )}
 </div>
 </div>
 </div>
 ...
 <button onClick={() => setShowShippingModal(false)} className="px-6 py-2 bg-muted/50 text-foreground/90 dark:bg-gray-700 dark:text-gray-100 rounded-lg hover:bg-gray-600">
 {t('iptal')}
 </button>
 <button

...
const { admin } = useAdmin();
 ...
 // 2. Actions
 if (method === 'whatsapp') {
 const message = `${t('wa_greeting')} ${selectedSupplier.name},\n\n${t('wa_order_list')}:\n` +
 items.map(i => `- ${i.quantity} ${i.unit} ${i.name}`).join('\n') +
 (orderNote ? `\n\n${t('wa_note')}: ${orderNote}` : '') +
 (deliveryDate ? `\n${t('wa_delivery')}: ${deliveryDate}` : '');
 ...
 {s.companyName && <p className="text-sm text-muted-foreground/80">{s.companyName}</p>}
 </button>
 ))}
 </div>
 )}
 ...
 {items.length === 0 ? (
 <div className="text-center text-muted-foreground/80 py-4">{t('listeniz_bos')}</div>
 ) : (
 ...
 <p className="text-sm text-muted-foreground">{formatLocalDate(new Date())}</p>
 </div>
 ...
 <span className="text-xs font-bold uppercase text-muted-foreground/80 block mb-1">{t('alici_tedari_kci')}</span>
 ...
 <th className="pb-2 w-24">{t('quantity')}</th>
 <th className="pb-2 w-20">{t('unit')}</th>
 ...
 {/* Notes */}
 <div className="mb-8">
 <label className="block text-xs font-bold uppercase text-muted-foreground/80 mb-1">{t('siparis_notu')}</label>
 <textarea
 value={orderNote}
 ...
 <div className="mb-8">
 <label className="block text-xs font-bold uppercase text-muted-foreground/80 mb-1">{t('teslimat_tarihi_i_stege_bagli')}</label>
 <input
 type="date"
 value={deliveryDate}
 onChange={e => setDeliveryDate(e.target.value)}
 className="bg-muted/30 border border-border/50 p-2 rounded"
 />
 </div>
 ...
 <button onClick={() => setStep(2)} className="w-full text-center text-muted-foreground/80 hover:text-foreground mt-4">{t('duzenlemeye_don')}</button>
 </div>
 )}
 </div>
 </div>

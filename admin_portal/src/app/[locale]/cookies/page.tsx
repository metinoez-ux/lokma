...
export default function CookiesPage() {
 ...
 <p className="text-muted-foreground/80 /60 mb-8">{t('desc')}</p>

 <div className="space-y-6 mb-12">
 <div className="bg-muted/30 dark:bg-background/5 border border-border/50 rounded-xl p-6">
 <div className="flex items-center justify-between mb-3">
 <div>
 <h3 className="font-bold text-lg">{t('necessary')}</h3>
 <p className="text-sm text-muted-foreground/80 /60">{t('necessaryDesc')}</p>
 </div>
 <div className="bg-green-500/20 text-green-500 px-3 py-1 rounded-full text-xs font-bold">{t('alwaysActive')}</div>
 </div>
 <p className="text-sm text-gray-400 /50">{t('necessaryDetail')}</p>
 </div>

 {(['analytics', 'marketing'] as const).map(key => (
 <div key={key} className="bg-muted/30 dark:bg-background/5 border border-border/50 rounded-xl p-6">
 <div className="flex items-center justify-between mb-3">
 <div>
 <h3 className="font-bold text-lg">{t(key)}</h3>
 <p className="text-sm text-muted-foreground/80 /60">{t(`${key}Desc`)}</p>
 </div>
 <button
 onClick={() => setPreferences({ ...preferences, [key]: !preferences[key] })}
 className={`w-12 h-6 rounded-full transition-all ${preferences[key] ? 'bg-[#ea184a]' : 'bg-background/20'}`}
 >
 <div className={`w-5 h-5 bg-background rounded-full transition-all ${preferences[key] ? 'ml-6' : 'ml-0.5'}`} />
 </button>
 </div>
 <p className="text-sm text-gray-400 /50">{t(`${key}Detail`)}</p>
 </div>
 ))}
 </div>
 ...
 }} className="bg-muted dark:bg-background/10 hover:bg-muted/50 dark:hover:bg-background/20 text-foreground px-8 py-4 rounded-xl font-bold transition-all">
 {t('acceptAll')}
 </button>
 </div>
 </div>

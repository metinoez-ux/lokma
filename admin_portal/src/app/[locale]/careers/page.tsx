...
export default function CareersPage() {
 ...
 <div key={b} className="bg-muted/30 dark:bg-background/5 border border-border/50 rounded-xl p-6 text-center">
 ...
 <p className="text-sm text-muted-foreground/80 /60">{t(`${b}d`)}</p>
 </div>
 ))}
 </div>
 ...
 className="flex flex-col md:flex-row md:items-center justify-between bg-muted/30 dark:bg-background/5 border border-border/50 rounded-xl p-6 hover:border-[#ea184a]/50 transition-all group"
 >
 <div className="mb-4 md:mb-0">
 <h3 className="font-bold text-lg group-hover:text-[#ea184a] transition-colors">{job.title}</h3>
 <p className="text-sm text-muted-foreground/80 /60">{job.department}</p>
 </div>
 <div className="flex flex-wrap gap-3">
 <span className="bg-muted dark:bg-background/10 px-3 py-1 rounded-full text-xs">{job.location}</span>
 ...
 <p className="text-muted-foreground/80 /60 mb-6">{t('notFoundDesc')}</p>
 <a

...
export default function KuryePage() {
 ...
 className="inline-flex items-center justify-center gap-3 bg-emerald-500 hover:bg-emerald-600 text-foreground px-12 py-5 rounded-2xl font-black text-xl shadow-2xl shadow-emerald-500/25 transition-all hover:scale-105 active:scale-95"
 >
 ...
 {benefits.map((b, i) => (
 <div key={i} className="bg-background dark:bg-[#0f172a] p-10 rounded-[2.5rem] border border-border/50 hover:border-emerald-500/50 transition-colors text-left flex flex-col group hover:-translate-y-2 duration-300 shadow-2xl shadow-black">
 ...
 <p className="text-muted-foreground/80 /60 text-lg leading-relaxed">{b.desc}</p>
 </div>
 ))}
 </div>
 </div>
 ...
 {/* Requirements */}
 <div className="bg-muted/30 dark:bg-background/[0.02] p-8 md:p-12 rounded-[3rem] border border-border/50 ">
 ...
 <li key={i} className="flex items-start gap-4 bg-muted dark:bg-black/40 p-5 rounded-2xl border border-border/50 hover:border-blue-500/30 transition-colors">
 <span className="material-symbols-outlined text-blue-400 shrink-0">check_circle</span>
 <span className="text-foreground/80 /80 font-medium text-lg leading-relaxed">{req}</span>
 </li>
 ))}
 </ul>
 </div>
 ...
 <div className="space-y-10 relative before:absolute before:inset-0 before:ml-7 before:-translate-x-px before:w-1 before:bg-background/10 before:z-0">
 {steps.map((step, i) => (
 <div key={i} className="relative z-10 flex items-start gap-8 group">
 <div className="w-14 h-14 shrink-0 bg-background dark:bg-[#0f172a] border-4 border-border/50 group-hover:border-[#ea184a] transition-colors rounded-full flex items-center justify-center font-black text-2xl text-foreground group-hover:text-[#ea184a]">
 {step.num}
 </div>
 <div className="pt-2">
 <h3 className="text-2xl font-bold mb-3">{step.title}</h3>
 <p className="text-muted-foreground/80 /60 text-lg leading-relaxed">{step.desc}</p>
 </div>
 </div>
 ))}
 </div>

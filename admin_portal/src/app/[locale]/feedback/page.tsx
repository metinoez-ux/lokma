...
function FeedbackContent() {
 ...
 <div className="bg-muted/30 dark:bg-background/5 border border-border/50 rounded-2xl shadow-lg p-8 max-w-md w-full text-center">
 <div className="text-6xl mb-4">✅</div>
 <h1 className="text-2xl font-bold mb-2">{t('thanks')}</h1>
 <p className="text-muted-foreground/80 /60 mb-6">{t('thanksSub')}</p>
 <div className="space-y-3">
 <Link href="/" className="block w-full bg-[#ea184a] text-white py-3 rounded-xl font-semibold hover:bg-red-600">{t('home')}</Link>
 <button onClick={() => { setSent(false); setMessage(''); setSubject(''); }} className="block w-full bg-muted dark:bg-background/10 text-foreground py-3 rounded-xl font-medium hover:bg-muted/50 dark:hover:bg-background/20">
 {t('newMsg')}
 </button>
 </div>
 </div>
 ...
 <div className="bg-muted/30 dark:bg-background/5 border border-border/50 rounded-2xl shadow-sm p-6">
 <h1 className="text-2xl font-bold mb-2">{t('subtitle')}</h1>
 <p className="text-muted-foreground/80 /60 mb-6">{t('desc')}</p>

 <form onSubmit={handleSubmit} className="space-y-6">
 <div>
 <label className="block text-sm font-medium text-muted-foreground /80 mb-2">{t('type')}</label>
 <div className="grid grid-cols-3 gap-2">
 {typeButtons.map(b => (
 <button key={b.key} type="button" onClick={() => setFeedbackType(b.key)}
 className={`py-3 px-4 rounded-xl border text-center transition ${feedbackType === b.key ? 'border-[#ea184a] bg-[#ea184a]/20 text-[#ea184a] ' : 'border-border/50 hover:border-border dark:hover:border-white/30'}`}>
 ...
 <div>
 <label className="block text-sm font-medium text-muted-foreground /80 mb-1">{t('email')}</label>
 <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={t('emailPh')}
 className="w-full px-4 py-3 bg-muted/30 dark:bg-background/5 border border-border/50 rounded-xl text-foreground placeholder-gray-400 dark:placeholder-white/40 focus:ring-2 focus:ring-[#ea184a] focus:border-transparent" />
 </div>
 ...
 className="w-full px-4 py-3 bg-muted/30 dark:bg-background/5 border border-border/50 rounded-xl text-foreground placeholder-gray-400 dark:placeholder-white/40 focus:ring-2 focus:ring-[#ea184a] focus:border-transparent resize-none" required />
 </div>
 ...
 <p className="text-center text-gray-400 /40 text-sm mt-6">{t('note')}</p>
 </div>
 </main>

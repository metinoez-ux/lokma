...
export default function ButcherOrdersPage() {
    ...
    // Reject order modal
    const [rejectingOrder, setRejectingOrder] = useState<ButcherOrder | null>(null);
    const [rejectReason, setRejectReason] = useState('');

    // Edit order modal
    const [editingOrderItems, setEditingOrderItems] = useState<ButcherOrder | null>(null);
    ...
    useEffect(() => {
        ...
        // Filter by businessId (new naming) - butcherId is for backward compat
        const q = query(
            ordersRef,
    ...
    const updateOrderStatus = async (orderId: string, newStatus: string, reason?: string) => {
        ...
        try {
            ...
            if (order) {
                ...
                try {
                    // Fetch customer's FCM token from users collection
                    const userDoc = await getDoc(doc(db, 'users', order.customerId));
                    const customerFcmToken = userDoc.data()?.fcmToken;
                        ...
                        if (notifyType) {
                            await fetch('/api/orders/notify', {
                                ...
                                body: JSON.stringify({
                                    ...
                                    customerFcmToken,
                                    butcherName: butcher?.companyName || 'Kasap',
                                }),
                            });
                            console.log('✅ Push notification sent for status:', newStatus);
                        }
                    }
                } catch (notifyError) {
                    console.error('Push notification error (non-blocking):', notifyError);
                }
            }
        } catch (error) {
    ...
    return (
        <div className="min-h-screen bg-gray-900">
            {/* Header - Red Bar like Butchers Page */}

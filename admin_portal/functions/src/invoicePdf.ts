 ...
 *
 * Generates GoBD-compliant German invoices as PDF buffers.
 * Supports both subscription and commission invoice types.
 */
...
function formatDate(ts: admin.firestore.Timestamp | Date): string {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.fontSize(7)
                ...
                .fillColor("#999999")
                .text(
                    `${COMPANY.name} · ${COMPANY.street} · ${COMPANY.city} · USt-IdNr.: ${COMPANY.taxId} · ${COMPANY.email}`,
                    col1,
                    footerY,

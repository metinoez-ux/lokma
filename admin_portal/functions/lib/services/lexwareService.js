...
exports.classifyTaxType = classifyTaxType;
exports.testLexwareConnection = testLexwareConnection;
exports.createLexwareInvoice = createLexwareInvoice;
exports.createLexwareCreditNote = createLexwareCreditNote;
exports.getLexwareInvoice = getLexwareInvoice;
exports.downloadLexwareInvoicePDF = downloadLexwareInvoicePDF;
exports.buildMonthlyInvoicePayload = buildMonthlyInvoicePayload;
// =============================================================================
 ...
 */
async function lexwareRequest(apiKey, method, path, body) {
 ...
 */
async function validateVatNumber(fullVatId) {
 ...
 */
async function classifyTaxType(params) {
 ...
 */
async function testLexwareConnection(apiKey) {
    try {
 ...
 */
async function createLexwareInvoice(apiKey, payload, finalize = true) {
    try {
        const queryParam = finalize ? "?finalize=true" : "";
        const { status, data } = await lexwareRequest(apiKey, "POST", `/v1/invoices${queryParam}`, payload);
 ...
 */
async function createLexwareCreditNote(apiKey, payload, finalize = true) {
    try {
        const queryParam = finalize ? "?finalize=true" : "";
        const { status, data } = await lexwareRequest(apiKey, "POST", `/v1/credit-notes${queryParam}`, payload);
        if (status === 200 || status === 201) {
            const createResponse = data;
 ...
 */
async function getLexwareInvoice(apiKey, invoiceId) {
    try {
 ...
 */
async function downloadLexwareInvoicePDF(apiKey, invoiceId) {
    try {
        ...
        if (renderResult.status === 200 && renderResult.data?.documentFileId) {
            // Download the file
            const { status, data } = await lexwareRequest(apiKey, "GET", `/v1/files/${renderResult.data.documentFileId}`);
 ...
 */
function buildMonthlyInvoicePayload(params) {
    const { businessName, businessAddress, vatId, periodStart, periodEnd, subscriptionPlanName, subscriptionFee, commissionAmount, commissionDetails, activeModules, sponsoredFee, sponsoredConversions, vatClassification, } = params;
...
//# sourceMappingURL=lexwareService.js.map
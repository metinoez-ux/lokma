...
async function saveAlexaCookie(businessId, cookie) {
...
async function initBusinessAlexa(businessId, cookie) {
    if (!AlexaRemote)
        return false;
    ...
    return new Promise((resolve) => {
        ...
        instance.init({ cookie: savedCookie, bluetooth: false, macDms: { device_private_key: "", adp_token: "" } }, async (err) => {
            if (err) {
            ...
            if (cookie)
                await saveAlexaCookie(businessId, cookie);
            console.log(`✅ Alexa connected for business ${businessId}`);
            resolve(true);
        });
    });
}
async function ensureAlexaReady(businessId) {
    if (alexaInstances.get(businessId)?.ready)
        return true;
    return initBusinessAlexa(businessId);
}
async function sendAlexaTTS(businessId, message) {
    const entry = alexaInstances.get(businessId);
    if (!entry?.ready)
        return false;
    return new Promise((resolve) => {
        entry.instance.sendSequenceCommand(null, "speak", message, (err) => {
            if (err) {
                console.error(`❌ Alexa TTS error:`, err.message);
                resolve(false);
            }
            else {
...
async function playAlexaSound(businessId, sound = "amzn_sfx_doorbell_chime") {
    const entry = alexaInstances.get(businessId);
    if (!entry?.ready)
        return false;
    return new Promise((resolve) => {
        entry.instance.sendSequenceCommand(null, "sound", sound, (err) => {
            if (err) {
                resolve(false);
            }
            else {
...
function getBusinessDevices(businessId) {
    const entry = alexaInstances.get(businessId);
    if (!entry?.ready)
        return [];
    const devices = entry.instance.getDevices() || {};
    return Object.values(devices)
        .filter((d) => d.deviceFamily === "ECHO" || d.deviceFamily === "KNIGHT")
        .map((d) => ({ name: d.accountName, serialNumber: d.serialNumber }));
}
// ─── WLED Service ────────────────────────────────────────────────────────────
async function flashWLED(devices, event) {
    if (devices.length === 0)
        return [];
...
function rgbToXY(r, g, b) {
    ...
    const sum = X + Y + Z;
    if (sum === 0)
        return [0.3128, 0.3290];
    return [X / sum, Y / sum];
}
async function flashHue(bridgeIp, username, lightIds, event) {
    if (!bridgeIp || !username || lightIds.length === 0)
        return false;
    const color = COLOR_PROFILES[event] || COLOR_PROFILES.new_order;
...
function buildOrderMessage(event, orderNumber, amount, items, language = "de-DE", currency = "EUR") {
...
// Express App
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
exports.iotApp = (0, express_1.default)();
exports.iotApp.use((0, cors_1.default)({ origin: true }));
exports.iotApp.use(express_1.default.json());
// ─── Health Check ────────────────────────────────────────────────────────────
exports.iotApp.get("/health", (_req, res) => {
    res.json({
        status: "ok",
...
exports.iotApp.post("/notify", async (req, res) => {
    const payload = req.body;
    if (!payload.businessId) {
        res.status(400).json({ error: "businessId is required" });
        return;
    }
    const event = (payload.event in COLOR_PROFILES ? payload.event : "new_order");
    const results = {};
...
exports.iotApp.post("/test", async (req, res) => {
...
exports.iotApp.post("/alexa/setup", async (req, res) => {
...
exports.iotApp.get("/alexa/status/:businessId", (req, res) => {
...
exports.iotApp.delete("/alexa/disconnect/:businessId", async (req, res) => {
    const bid = req.params.businessId;

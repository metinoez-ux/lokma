...
exports.onReservationStatusChange = (0, firestore_1.onDocumentUpdated)({
    ...
    if (!statusChanged && !tableNumbersChanged)
        return;
    const newStatus = after.status;
    const businessId = event.params.businessId;
    ...
    try {
        ...
        console.error("Error fetching biz address", e);
    }
    ...
    if (newStatus !== "confirmed" && newStatus !== "rejected" && newStatus !== "reactivated" && newStatus !== "cancelled")
        return;
    ...
    if (!isTableAssignmentOnly && (newStatus === "confirmed" || newStatus === "rejected" || newStatus === "cancelled")) {
        try {
            ...
            if (Array.isArray(preOrderItems) && preOrderItems.length > 0) {
                const itemsListHtml = preOrderItems.map((item) => {
                    const qty = item.quantity || 1;
                    const pName = item.product?.name || item.name || "Ürün";
                    // Options
                    let optionsTxt = "";
                    if (Array.isArray(item.selectedOptions) && item.selectedOptions.length > 0) {
                        optionsTxt = `<div class="preorder-item-options">+ ${item.selectedOptions.map((o) => o.name || "").filter(Boolean).join(", ")}</div>`;
                    }
                    return `
...
Object.defineProperty(exports, "onUserAccountDeleted", { enumerable: true, get: function () { return accountDeletion_1.onUserAccountDeleted; } });
// Kermes: Garson/Kurye Atama & Timeout
var kermesWaiterFunctions_1 = require("./kermesWaiterFunctions");
Object.defineProperty(exports, "kermesWaiterTimeoutCheck", { enumerable: true, get: function () { return kermesWaiterFunctions_1.kermesWaiterTimeoutCheck; } });
Object.defineProperty(exports, "onKermesOrderReady", { enumerable: true, get: function () { return kermesWaiterFunctions_1.onKermesOrderReady; } });
...
var imageOptimizer_1 = require("./imageOptimizer");

...
const map = JSON.parse(fs.readFileSync('business_map.json', 'utf8'));
...
files.forEach(filePath => {
    ...
    if (!code.includes("const t = useTranslations('AdminBusiness');")) {
        const componentNames = ['BusinessesPage', 'BusinessDetailsPage'];
        for(let name of componentNames) {
           const decl = `export default function ${name}() {`;
